...
class Player {
    ...
    update() {
        ...
        this.onGround = false; // Asumir que está en el aire hasta que se demuestre lo contrario

        // Colisión con el suelo del canvas
...
function loadLevel(levelIndex) {
    ...
    enemies = levelData.enemies.map(e => new Enemy(e.startX, e.y, e.range));
    goal = levelData.goal;

    levelDisplay.textContent = `Nivel: ${levelIndex + 1}`;
    timer = LEVEL_TIME;
    timerDisplay.textContent = `Tiempo: ${timer}`;
    messageDisplay.textContent = '¡Vamos allá!';
}
...
function nextLevel() {
    ...
    if (currentLevel < levels.length) {
        ...
        gameOver('¡HAS GANADO EL JUEGO! Eres la bestia.');
    }
}
...
function gameOver(message) {
    clearInterval(gameInterval);
    clearInterval(timerInterval);
    messageDisplay.textContent = `${message} (Pulsa Enter para reiniciar)`;
    ...
    enemies.forEach(enemy => {
        ...
        if (player.x < enemy.x + enemy.width &&
            ...
            if (player.velocityY > 0 && player.y + player.height < enemy.y + 20) {
                enemy.active = false;
                player.velocityY = -6; // Pequeño rebote
            } else {
                gameOver('¡Has perdido! Te ha tocado un enemigo.');
            }
        }
    });
